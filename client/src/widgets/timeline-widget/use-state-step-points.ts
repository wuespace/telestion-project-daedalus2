...
function hasEjectorNominalState(
	ejectorState: number,
	minState: number,
	delay = 0
): boolean {
	return delay <= MAX_DELAY && ejectorState >= minState && ejectorState <= 18;
}
...
export function useStateStepPoints(): StepPoint[] {
	const states = useExperimentStates();
	const [seedAState, seedBState, ejectorState] = states;
	const delays = useExperimentDelay();
	const [seedADelay, seedBDelay, ejectorDelay] = delays;

	const isAnyInRadioSilence =
		isInRadioSilence(seedAState) ||
		isInRadioSilence(seedBState) ||
		isInRadioSilence(ejectorState);

	const isAnyActiveInRadioSilence =
		isInRadioSilence(seedAState, seedADelay) ||
		isInRadioSilence(seedBState, seedBDelay) ||
		isInRadioSilence(ejectorState, ejectorDelay);

	const isAnyDelayed =
		(isDelayed(seedADelay) && !isInRadioSilence(seedAState)) ||
		(isDelayed(seedBDelay) && !isInRadioSilence(seedBState)) ||
		isDelayed(ejectorDelay);

	const isDiverged =
		hasDiverged(seedAState, seedBState) ||
		(hasDiverged(seedAState, ejectorState) && ejectorState < 16) ||
		(hasDiverged(seedBState, ejectorState) && ejectorState < 16);

	const isAnyInAbnormalState =
		isInAbnormalState(seedAState) ||
		isInAbnormalState(seedBState) ||
		isInAbnormalState(ejectorState);
	...
	return [
		...
		},
		{
			...
			isDone:
				...
				hasEjectorNominalState(ejectorState, 4) ||
				isAnyInRadioSilence,
			isActive:
				isIn(seedAState, 4, 6, seedADelay) ||
				isIn(seedBState, 4, 6, seedBDelay) ||
				isIn(ejectorState, 4, 6, ejectorDelay) ||
		...
		},
		{
			...
			isDone:
				...
				hasEjectorNominalState(ejectorState, 7),
			isActive:
				isIn(seedAState, 7, 9, seedADelay) ||
				isIn(seedBState, 7, 9, seedBDelay) ||
				isIn(ejectorState, 7, 9, ejectorDelay),
			isCritical: false
		},
		{
			id: 4,
			...
			isDone:
				...
				hasEjectorNominalState(ejectorState, 10),
			isActive:
				isIn(seedAState, 10, 12, seedADelay) ||
				isIn(seedBState, 10, 12, seedBDelay) ||
				isIn(ejectorState, 10, 12, ejectorDelay),
			isCritical: false
		},
		{
			id: 5,
			...
			isDone:
				...
				hasEjectorNominalState(ejectorState, 13),
			isActive:
				isIn(seedAState, 13, 15, seedADelay) ||
				isIn(seedBState, 13, 15, seedBDelay) ||
				isIn(ejectorState, 13, 15, ejectorDelay),
			isCritical: false
		},
		{
			id: 6,
			...
			isDone:
				...
				hasEjectorNominalState(ejectorState, 16),
			isActive:
